

--------------------------------------------------------------------------------------------------------
Name			Student ID		Roles		Role-based 		Overall
								Contribution(%) 	Contribution(%)
--------------------------------------------------------------------------------------------------------
Anshul R      		2013A7PS114G            Unit Tests          80%                    15%
						Testing             80%  
						Code Dev.	    10%
						Proj. Requirements  30%			    			

Janani Durairaj	        2012B1A7735G  		CRC cards 	    50%                     15%
						Use cases 	    50% 	

Vishnu Nair	        2012A3PS079		CRC cards 	    50%                     15%
						Use cases 	    50%
						JavaDocs	    15%

Aditya Singh		2013A7PS098G		UML		    70%			    15%
						

Shatrujit Kumar		2013A7PS044G		Code Dev.	    15%			    15%
						JavaDocs	    85%	
						Unit Tests	    20%		

Rohit Pandey		2013A7PS711G		Proj. Requirements  55%			    15%
						Code Dev.   	    75%		
						Testing		    20%

Dharm S Jain		2013A7PS696G		Project Manager	    25%			    10%
						UML		    50%			   							
-------------------------------------------------------------------------------------------------------



Unit Tests
------------------------------------------------------------------------
---------------------------------------------------------------------------
Topic						Number		Test Status
						of tests	Pass	Fail
----------------------------------------------------------------------------

EventTest					  2		 2	 0
MessageTest					  5		 4	 1	(Fixed)
PersonTest					  2		 2	 0
CoreTest					  3		 3	 0



Tests added later
-------------------------------------------------------------------------------------------
ProjectHeadTest					  2		 2	0
VolunteerTest					  1		 1	0
-------------------------------------------------------------------------------------------



B. Installation Istructions :

	1. Paste the JAR file in the desired location.

	2. For Windows users : Click the 'runApplication.bat'
	   For Linux users   : In the terminal, bash the 'runApplication.sh' file.
